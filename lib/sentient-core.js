...
class SentientObserver {
    ...
    processAdaptive(input, options = {}) {
        ...
        // Helper: convert any state-like object to a SparsePrimeState
        // coherenceGatedCompute internally calls computeCoherence which
        // requires state.getActivePrimes() — only SparsePrimeState has it.
        const toSparse = (semanticState) => {
            const sparse = new SparsePrimeState(this.primeCount, this.primeCount);
            // Handle PrimeState (.amplitudes), stub ({state: Map}), or anything with a Map
            const stateMap = semanticState && (semanticState.amplitudes || semanticState.state);
            if (stateMap && typeof stateMap.forEach === 'function') {
                stateMap.forEach((amp, prime) => {
                    if (amp && typeof amp !== 'undefined') {
                        sparse.set(prime, amp);
                    }
                });
            }
            return sparse;
        ...
        // Create initial state as SparsePrimeState (must have getActivePrimes).
        // textToOrderedState returns a Hypercomplex number, not a Map-based state,
        // so we use SparsePrimeState.fromHash for text-based inputs.
        let state;
        if (input.primeState && typeof input.primeState.getActivePrimes === 'function') {
            // Already a SparsePrimeState — use directly
            state = input.primeState;
        } else if (input.primeState && (input.primeState.state || input.primeState.amplitudes)) {
            // PrimeState (.amplitudes Map) or stub ({state: Map}) — convert
            state = toSparse(input.primeState);
        } else if (input.content && typeof input.content === 'string') {
            // Hypercomplex result from textToOrderedState — use fromHash instead
            state = SparsePrimeState.fromHash(input.content, this.primeCount, Math.min(32, this.primeCount));
        } else if (typeof input === 'string') {
            state = SparsePrimeState.fromHash(input, this.primeCount, Math.min(32, this.primeCount));
        } else {
        ...
        const stepFn = (currentState, stepNum) => {
            ...
            const stateMap = currentState && (currentState.activations || currentState.state);
            if (stateMap && typeof stateMap.forEach === 'function') {
                for (const prime of this.primes) {
                    const amp = stateMap.get(prime);
                    const ampVal = amp && (amp.amplitude || amp);
                    if (ampVal) {
                        const osc = this.prsc.getOscillator(prime);
                        if (osc) {
                            const magnitude = typeof ampVal.norm === 'function' ? ampVal.norm() : Math.abs(ampVal);
            ...
            // Return as SparsePrimeState so computeCoherence can call getActivePrimes()
            return toSparse(semanticState);
        };
