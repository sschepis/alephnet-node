...
describe('tinyaleph loader', () => {
  ...
  it('classifies stability and lyapunov estimates deterministically', () => {
    // Runtime-verified classes: strong convergence -> 'collapsed', weak
    // divergence -> 'unstable', strong divergence -> 'chaotic'.
    expect(kernel.classifyStability(-0.5)).toBe('collapsed');
    expect(kernel.classifyStability(-0.05)).toBe('stable');
    expect(kernel.classifyStability(0.01)).toBe('unstable');
